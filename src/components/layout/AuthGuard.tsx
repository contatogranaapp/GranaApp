...
export function AuthGuard({ children }: Props) {
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', backgroundColor: '#0C0C0F', overflow: 'hidden' }}>
      <SidebarWrapper profile={profile} />
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        <MobileHeaderWrapper profile={profile} />
        <main style={{
          flex: 1,
          overflowY: 'auto',
...
function MobileHeaderWrapper({ profile }: { profile: any }) {
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const { MobileHeader } = require('@/components/layout/MobileHeader')
  return <MobileHeader profile={profile} />
}
